        ...
        StringRequest request = new StringRequest(Constants.GetUrl(this)+"="+Constants.GetKey(this), string -> {
            try {
                ...
                if (((String) object.get("status")).equalsIgnoreCase("success")) {
                    ...
                    int divide = (MyApp.SCREEN_WIDTH)/Utils.dp2px(SeriesPlayActivity.this,result.width());
                    if(divide<1){
                        Log.e("rss1",rss);
                        if(is_msg){
...
//                            txt_rss.startAnimation(AnimationUtils.loadAnimation(getApplicationContext(), R.anim.marquee1));
                    ...
                    Toast.makeText(SeriesPlayActivity.this, "Server Error!", Toast.LENGTH_SHORT).show();
                }
        ...
        RequestQueue rQueue = Volley.newRequestQueue(SeriesPlayActivity.this);
        rQueue.add(request);
    }
    int rss_time;
